...
static double heuristic(double x0, double y0, double goal_x, double goal_y){
  return sqrt((x0 - goal_x) * (x0 - goal_x) + (y0 - goal_y) * (y0 - goal_y));
}
...
vector<HBF::maze_s> HBF::expand(HBF::maze_s state, std::vector<int> & goal) {
  int g = state.g;
  double x = state.x;
  double y = state.y;
  ...
  for(double delta_i = -35; delta_i < 40; delta_i+=5)
  {
    ...
    state2.y = y2;
    state2.theta = theta2;
    state2.f = g2 + heuristic(x2, y2, (double)goal[0], (double)goal[1]);
...
HBF::maze_path HBF::search(vector< vector<int> > grid, vector<double> start, vector<int> goal) {
  ...
  */
  ...
  maze_s state;
  state.g = g;
  state.f = heuristic(start[0], start[1], (double)goal[0], (double)goal[1]);
  state.x = start[0];
  state.y = start[1];
  state.theta = theta;
  ...
  while(!opened.empty())
  {
    ...
    [](const maze_s & a, const maze_s & b) -> bool 
    {
        return a.f < b.f;
    });
