...
std::vector<double> getXY(double s, double d, const std::vector<double> &maps_s, const std::vector<double> &maps_x,
    ...
    while (s > maps_s[prev_wp + 1] && (prev_wp < (int)(maps_s.size() - 1))) { prev_wp++; }
...
LaneNumber MapFrenetDToLane(int d) {
  if (d <= 0 || d >= 12) return LaneInvalid;

  if(d > 0 && d < 4) return LaneLeft;
  if(d > 4 && d < 8) return LaneMiddle;
  if(d > 8 && d < 12) return LaneRight;
  return LaneInvalid;
}
