...
Traffic::Traffic() {
    ...
    lane_state = std::vector<LaneState>(LaneNumberTotal, LaneState());  
};

Traffic::~Traffic(){};

void Traffic::UpdateTrafficState(const std::vector<std::vector<double>> &sensor_fusion, int ego_s, int ego_d) {
    ...
    for (int i = 0; i < sensor_fusion.size(); i++) {
        ...
        const double frenet_s = sensor_fusion[i][JSON_INDEX_SENSOR_FUSION_VEHICLE_S];
        ...
        lane_state[veh_lane].vehicles[lane_slot] =  
          TrafficVehicle(id, s_diff, frenet_d, speed);
    ...
    for(int lane = 0; lane < LaneNumberTotal; lane++) {
      ...
      const double avg_speed = num_vehicles == 0 ? 0.0: speed_sum / num_vehicles;
      
      lane_state[lane].average_speed = avg_speed;

      // No other vechiles in this lane, use very large S to denote very large free space
      double free_space_start = MAX_S;
      double free_space_end = -MAX_S;
      ...
      for(int i = 0; i < num_vehicles; i++) {
        ...
        if(vehicle.frenet_s > 0) {
          ...
          if(i != 0) {
            ...
            if(vehicle2.frenet_s < 0)
              free_space_end = vehicle2.frenet_s;
          }
          break;
        }
      }
